...
import jp.co.soramitsu.feature_wallet_impl.data.network.blockchain.extrinsics.TransferRequest2
...
import jp.co.soramitsu.feature_wallet_impl.data.network.blockchain.struct.SignedExtrinsic2.accountId2
import jp.co.soramitsu.feature_wallet_impl.data.network.blockchain.struct.SignedExtrinsic2.call2
import jp.co.soramitsu.feature_wallet_impl.data.network.blockchain.struct.SignedExtrinsic2.signature2
...
import jp.co.soramitsu.feature_wallet_impl.data.network.blockchain.struct.SubmittableExtrinsic2.byteLength2
...
class WssSubstrateSource(
    ...
    override fun performTransfer2(
            account: Account,
            transfer: Transfer,
            keypair: Keypair
    ): Single<String> {
        return buildSubmittableExtrinsic2(account, keypair).map { extrinsic ->
            TransferRequest2(extrinsic)
        }.flatMap { transferRequest ->
            socketService.executeRequest(transferRequest,
                    responseType = string().nonNull(),
                    deliveryType = DeliveryType.AT_MOST_ONCE
    ...
    private fun buildSubmittableExtrinsic2(
            account: Account,
            keypair: Keypair
    ): Single<EncodableStruct<SubmittableExtrinsic2>> {
        return getRuntimeVersion().flatMap { runtimeInfo ->
            val cryptoType = mapCryptoTypeToEncryption(account.cryptoType)
            val accountIdValue = getAccountId(account.address)
            ...
            getNonce(account).map { currentNonce ->
                ...
                val callOrderScale = createOrderScaleCall(account.network.type, 1.toBigInteger(), 34.toBigInteger(),
                        0.toBigInteger(), 1000000000000.toBigInteger())
                val payload = ExtrinsicPayloadValue2 { payload ->
                    payload[ExtrinsicPayloadValue2.call] = callOrderScale
                    payload[ExtrinsicPayloadValue2.nonce] = currentNonce
                    payload[ExtrinsicPayloadValue2.specVersion] = runtimeInfo.specVersion.toUInt()
                    payload[ExtrinsicPayloadValue2.transactionVersion] = runtimeInfo.transactionVersion.toUInt()

                    payload[ExtrinsicPayloadValue2.genesis] = genesisBytes
                    payload[ExtrinsicPayloadValue2.blockHash] = genesisBytes
                }
                val signatureValue = Signature(
                        encryptionType = cryptoType,
                        value = signer.signExtrinsic2(payload, keypair, cryptoType)
                )

                val extrinsic = SignedExtrinsic2 { extrinsic ->
                    extrinsic[accountId2] = accountIdValue
                    extrinsic[signature2] = signatureValue
                    extrinsic[SignedExtrinsic2.nonce] = currentNonce
                    extrinsic[call2] = callOrderScale
                }

                val extrinsicBytes = SignedExtrinsic2.toByteArray(extrinsic)
                val byteLengthValue = extrinsicBytes.size.toBigInteger()

                val submittableExtrinsic = SubmittableExtrinsic2 { struct ->
                    struct[byteLength2] = byteLengthValue
                    struct[signedExtrinsic2] = extrinsic
                }
...
//            }
    ...
    private fun createOrderScaleCall(
            networkType: Node.NetworkType,
            collectionId: BigInteger,
            itemId: BigInteger,
            value: BigInteger,
            price: BigInteger
    ): EncodableStruct<Call2> {
        return Call2 { call ->
