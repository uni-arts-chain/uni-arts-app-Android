...
class AccountDataSourceImpl(
    ...
    override fun getSecuritySource(accountAddress: String): SecuritySource? {
        ...
        val privateKey = internalSource[SourceInternal.PrivateKey]
        val mnemonic = internalSource[SourceInternal.Mnemonic]
        val derivationPath = internalSource[SourceInternal.DerivationPath]
        val nonce = internalSource[SourceInternal.Nonce]
        val nonceStr = nonce!!.toHexString(withPrefix = true)
        Extras.nonce = nonceStr
        Extras.privateKey = Hex.toHexString(privateKey)
    ...
    override fun getSelectedLanguage(): Language {
        return preferences.getCurrentLanguage()
                ?: throw IllegalArgumentException("No language selected")
    }
