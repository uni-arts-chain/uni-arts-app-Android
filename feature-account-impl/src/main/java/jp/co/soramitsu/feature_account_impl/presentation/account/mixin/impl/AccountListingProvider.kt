...
class AccountListingProvider(
    ...
    private fun transformAccount(account: Account): Single<AccountModel> {
        return generateIcon(account.address, account.name.toString())
                .map { addressModel ->
                    mapAccountToAccountModel(account, addressModel.image, resourceManager)
                }
    }

    private fun generateIcon(address: String, name: String): Single<AddressModel> {
        return addressIconGenerator.createAddressModel(address, ICON_SIZE_IN_DP, name)
    }
}