...
class PinCodeViewModel(
    ...
    private fun authSuccess() {
        when (pinCodeAction) {
            ...
            is PinCodeAction.Check -> {
                var id = pinCodeAction.toolbarConfiguration.titleRes
                var title = id?.let { resourceManager.getString(it) }
//                require(pinCodeAction.delayedNavigation is NavComponentDelayedNavigation)
                if (title.equals(resourceManager.getString(R.string.pricate_key_export))) {
                    EditWalletFragment.pincodeCheck = true;
                    router.back()
                }
//                else if (pinCodeAction.delayedNavigation.globalActionId == R.id.action_open_main) {
//
//                }
                else
                    router.openAfterPinCode(pinCodeAction.delayedNavigation)
//                pinCodeAction.toolbarConfiguration.titleRes?.let {
////
////                    if (it.equals(R.string.pricate_key_export)) {
////                        router.back()
////                    } else
////                        router.openAfterPinCode(pinCodeAction.delayedNavigation)
////                }
