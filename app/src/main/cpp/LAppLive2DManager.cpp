...
void LAppLive2DManager::ChangeSceneWithPath(std::string modelJsonPath, std::string modelJsonName) {
    if (DebugLogEnable)
    {
        LAppPal::PrintLog("[APP]model index: %d", _sceneIndex);
    }
...
//    std::string modelJsonName = ModelDir[index];
//    modelJsonName += ".model3.json";
    std::string modelPath = modelJsonPath;
    std::string tempModelJsonName = modelJsonName;
    ReleaseAllModel();
    _models.PushBack(new LAppModel());
    _models[0]->LoadAssets(modelPath.c_str(), tempModelJsonName.c_str());
