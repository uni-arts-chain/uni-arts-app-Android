...
//    }
    ...
    x = width * 0.1f;
    y = height - powerTexture->height;
    fWidth = static_cast<float>(powerTexture->width * 3.0f);
    fHeight = static_cast<float>(powerTexture->height * 3.0f);

    if(_close == NULL)
    {
        _close = new LAppSprite(x, y, fWidth, fHeight, powerTexture->id, _programId);
    }
    else
    {
        _close->ReSize(x, y, fWidth, fHeight);
    }
...
//        LAppLive2DManager::GetInstance()->NextScene();
        if (_modelPath != "") {
            LAppLive2DManager::GetInstance()->ChangeSceneWithPath(_modelPath, _jsonModelName);
        }
    }
...
void LAppView::LoadModelFile(std::string modelPath, std::string jsonModelName) {
//    LAppLive2DManager::GetInstance()->ChangeSceneWithPath(modelPath, jsonModelName);
